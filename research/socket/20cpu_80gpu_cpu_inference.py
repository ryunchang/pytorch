...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        rcv = client_socket.recv(5760000)
        rcv = np.frombuffer(rcv, dtype=np.float32)
        ...
        snd = x.to("cpu").numpy().tobytes()
        client_socket.send(snd)
        rcv = client_socket.recv(1024000)
        rcv = np.frombuffer(rcv, dtype=np.float32)
...
def main():
    ...
    test_batch_size=16
    log_interval =100
    cpu_pth_path = "/home/yoon/Yoon/pytorch/research/socket/cpu.pth"
    gpu_pth_path = "/home/yoon/Yoon/pytorch/research/socket/gpu.pth"
